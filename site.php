...
$app->get("/products/:desurl", function ($desurl) {
	...
	$page->setTpl("product-detail", [
		...
		'category' => $product->getCategoryByProduct()
	]);
});
...
$app->get("/cart/:idproduct/remove", function ($idproduct) {
	...
	$cart->removeProductToCart($product, true);
...
$app->post("/forgot/reset", function () {
	...
	$user->updateUser($_POST['password']);
...
$app->get("/boleto/:idorder", function ($idorder) {
	...
	$dadosboleto["endereco1"] = $order->getdesaddress() ."-".$order->getdescomplement();
	$dadosboleto["endereco2"] = $order->getdescity() ."-". $order->getdescity() ."-" . $order->getdeszipcode() ;
...
$app->post("/profile/change-password",function(){
	...
	if(!isset($_POST['new_pass_confirm']) || $_POST['new_pass_confirm'] ==''){
		...
		User::setError('confirme nova senha');
		header("Location: /profile/change-password");
		exit;
	...
	if($_POST['new_pass_confirm'] != $_POST['new_pass'] ){
		...
		User::setError('A confirmação e a nova senha devem ser iguais');
		header("Location: /profile/change-password");
		exit;
	...
	$user->updateUser($_POST['new_pass']);
